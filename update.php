...
if(
  ...
  !isset($_POST["jsonFlg"]) || $_POST["jsonFlg"]==""
){
  exit('ParamError');
}
...
$stmt = $pdo->prepare("UPDATE natalie_news_table SET news_id=:id, news_title=:title, news_contents=:contents, json_flg=:jsonFlg WHERE news_id=:id");
$stmt->bindValue(':id', $newsId);
$stmt->bindValue(':title', $newsTitle);
$stmt->bindValue(':contents', $newsContents);
$stmt->bindValue(':jsonFlg', $jsonFlg);
$status = $stmt->execute();
